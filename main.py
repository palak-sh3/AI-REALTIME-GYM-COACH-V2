...
def build_voice_pipeline():
    ...
    if not api_key:
        raise ValueError(
            "GROQ_API_KEY not found. Put it in .env as GROQ_API_KEY=your_key"
        )

    groq_client = Groq(api_key=api_key)
    llm_coach = LLMCoach(groq_client)
    tts = TextToSpeech()
    return VoicePipeline(llm_coach, tts)
...
def main():
    ...
    if "voice_pipeline" not in st.session_state:
        try:
            st.session_state.voice_pipeline = build_voice_pipeline()
        except Exception as e:
            st.session_state.voice_pipeline = None
            st.error(f"Voice coach initialization failed: {e}")

    workout_started = st.session_state.get("workout_started", False)

    with st.sidebar:
        st.title("🏋️‍♂️ Real-Time AI Coach")
        ...
        if workout_started:
            ...
            elif exercise == "Biceps Curl (Dumbbell)":
